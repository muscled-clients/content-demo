...
interface TenthSlideProps {
  navigateToSlide?: (slideIndex: number) => void;
}

export default function TenthSlide({ navigateToSlide }: TenthSlideProps) {
  ...
  const command = 'Please refactor this code with modern patterns and best practices';
  
  // Bad code with poor patterns, repetition, and inefficiencies
  const badCode = `function getUserData() {
  var users = [];
  ...
  // Refactored code with modern patterns and best practices
  const refactoredCodeLines = [
    '// Refactored with async/await, destructuring, and modern patterns',
    '',
    ...
    '    ',
    '    // Filter active adult users with clear, readable logic',
    '    const activeAdultUsers = users.filter(',
    '      ({ active, age }) => active && age > 18',
    '    );',
    '    ',
    '    // Update UI using modern DOM methods',
    '    updateUserCount(activeAdultUsers.length);',
    '    renderUserList(activeAdultUsers);',
    '    ',
    ...
    '  if (errorElement) {',
    '    errorElement.textContent = \'Unable to load users. Please try again.\';',
  ...
  const renderHighlightedLine = (line: string, index: number) => {
    ...
    const processLine = () => {
      ...
      const patterns: Array<{
        ...
        { regex: /^(const|let|var|function|async|await|return|if|else|try|catch|finally|throw|new|import|export|default)\b/, color: '#569CD6', bold: true },
        { regex: /^(true|false|null|undefined)\b/, color: '#569CD6', bold: true },
        
        // Built-in objects and methods
        { regex: /^(document|console|DocumentFragment)\b/, color: '#4FC1FF', bold: true },
        { regex: /^(forEach|filter|map|push|appendChild|createElement)\b/, color: '#DCDCAA' },
        
        // Numbers
        { regex: /^\d+/, color: '#B5CEA8', bold: true },
        
        // Identifiers (must come after keywords)
        { regex: /^[A-Z][a-zA-Z0-9]*/, color: '#4EC9B0', bold: true },  // Classes
        { regex: /^[a-z_][a-zA-Z0-9_]*/, color: '#9CDCFE' },  // Variables/functions
        
        // Arrow functions
        { regex: /^=>/, color: '#569CD6', bold: true },
  ...
  return (
    <div className="w-full h-full flex relative overflow-hidden bg-gray-950">
      ...
      <div className="flex-1 flex flex-col items-center px-8 z-10 pt-6">
        ...
        >
          <h1 className="text-3xl font-light text-white mb-1">Legacy to Modern</h1>
          <p className="text-gray-400 text-base">Claude's Intelligent Code Refactoring</p>
        </motion.div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-sm">
              ...
              <motion.div
                ...
                className="mb-4"
              >
                <div className="text-gray-500 mb-2">// app.js - Legacy code with poor patterns</div>
                <div className="text-red-400 text-xs mb-2">⚠️ Code issues: callback hell, var usage, DOM manipulation, no error handling</div>
                <div className="bg-gray-800/50 p-3 rounded border border-red-900/30">
                  <pre className="text-gray-400 whitespace-pre-wrap">{badCode}</pre>
                </div>
              </motion.div>
                ...
                >
                  <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Analyzing patterns and applying modern best practices...</span>
                    </div>
                  </div>
                </motion.div>
              )}
                ...
                >
                  <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-3 mb-4">
                    <div className="text-green-400 flex items-center gap-2">
                      ...
                      <span>Code refactored with modern patterns!</span>
                    </div>
                    <div className="text-green-400 text-xs mt-2">
                      ✅ Async/await ✅ Destructuring ✅ Arrow functions ✅ Error handling ✅ Separation of concerns
                    </div>
                  </div>
                  <div className="text-gray-500 mb-2">// app.js - Refactored with modern JavaScript</div>
                  <div className="bg-gray-800/50 p-4 rounded border border-gray-700 overflow-x-auto">
                    {refactoredCodeLines.map((line, index) => renderHighlightedLine(line, index))}
                  </div>
                </motion.div>
              )}
